...
export default function AdminContactsRoute() {
  ...
  const load = async (nextStatus: "unfinished" | "finished") => {
      ...
      <div className="space-y-3">
        {contacts.map((contact) => (
          <article key={contact.id} className="rounded-2xl border border-white/10 bg-black/25 p-4">
            ...
            <button
              onClick={async () => {
                await fetch("/api/admin/contacts", {
                  ...
                  body: JSON.stringify({
                    id: contact.id,
                    status: contact.status === "unfinished" ? "finished" : "unfinished",
                  }),
                });
                await load(status);
              }}
            ...
            >
              {contact.status === "unfinished" ? "Mark Finished" : "Mark Unfinished"}
            </button>
          </article>
        ))}
      </div>
